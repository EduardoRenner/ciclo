/**
 * Mantém a AGENDA FUTURA da barbearia de demonstração (`dom-rocha`) sempre viva em relação a hoje.
 *
 *   node scripts/seed-demo-agenda-futura.mjs
 *
 * O histórico semeado do `dom-rocha` parou num dia fixo; passadas duas semanas, "Hoje" e "Agenda"
 * abrem VAZIAS — e a agenda é o coração do produto. Este script apaga os agendamentos futuros
 * (pending/confirmed) do tenant e recria os próximos 21 dias úteis com uma ocupação plausível:
 * chão de ~65%, três profissionais, respeitando horário de funcionamento, duração de cada serviço
 * e a trava `appointments_no_overlap` (as cadeias por profissional são sequenciais, nunca colidem).
 *
...
const { data: tenant } = await svc.from('tenants').select('id, timezone').eq('slug', 'dom-rocha').single()
const T = tenant.id
...
for (let d = 0; d < 21; d++) {
  ...
  for (const prof of profs) {
    ...
    while (minuto < fim - 30 && guarda++ < 40) {
      ...
      const clientId = pick(`cli${d}${prof.id}${minuto}`, usarCiclo ? naHora : todos)
      ...
      novos.push({
        ...
        created_by: online ? null : dono.user_id,
