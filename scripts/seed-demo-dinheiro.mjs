/**
 * Preenche a CAMADA DE DINHEIRO da barbearia de demonstração (`dom-rocha`): uma comanda fechada
 ...
 *
 *   node scripts/seed-demo-dinheiro.mjs
 *
...
const { data: tenant, error: et } = await svc.from('tenants').select('id').eq('slug', SLUG).single()
precisa(et, 'buscar tenant')
const T = tenant.id
...
// Idempotência: derruba só as comandas presas a atendimentos DESTE tenant.
const idsAtend = atendimentos.map((a) => a.id)
const { data: ticketsAntigos } = await svc.from('tickets').select('id').eq('tenant_id', T).in('appointment_id', idsAtend)
...
console.log(`${tickets.length} comandas fechadas, ${itens.length} itens${semServico ? ` (${semServico} atend. sem serviço, pulados)` : ''}`)
...
console.log(`${espera.length} na fila de espera (2 já avisados)`)

// ── Avaliações ────────────────────────────────────────────────────────────────────────────────
const COMENTARIOS = [
  'Atendimento impecável, saí no horário certinho.',
  'Melhor corte que já fiz na cidade.',
  'Ambiente tranquilo e o Rafael caprichou na barba.',
...
console.log('')
console.log(`faturamento (receita do salão): R$ ${(receita / 100).toFixed(2)}`)
console.log(`lucro acumulado:                R$ ${(lucro / 100).toFixed(2)}  (${((lucro / receita) * 100).toFixed(0)}% de margem)`)
if (erros) {
  console.error(`\n✗ ${erros} inconsistências nas comandas — algo está errado, revise antes de usar`)
  process.exit(1)
}
console.log('\n✓ comandas consistentes: profit = fórmula do core, total = receita + gorjeta, nada fechado no futuro')
