...
export default async function PaginaComanda({ params }: { params: Promise<{ id: string }> }) {
  ...
  const comandaPromise = buscarComanda(db, ctx.tenantId, id)

  /*
   * Encadeado na PRÓPRIA `comandaPromise` (nunca uma segunda chamada a `buscarComanda`), e não
   * dentro do `Promise.all` de baixo esperando ele terminar todo: `sobra` só depende de
   * `ticket`/`items`, nunca de `servicos`, `produtos` ou `plano`. Mesma classe de achado do loop
   * de performance corrigido em `admin/caixa/page.tsx` (docs/DECISOES.md) — comida por uma
   * dependência falsa, não pela dependência real.
   *
   ...
   */
  const sobraPromise = comandaPromise.then(async ({ ticket, items }) => {
    if (!(podeVerLucro && (ticket.status === 'closed' || ticket.status === 'paid'))) {
      return { sobra: null, destinoDaFicha: CATALOGO_DE_SERVICOS }
    }
  ...
  const [{ ticket, items }, servicos, produtos, plano, { sobra, destinoDaFicha }] = await Promise.all([
    comandaPromise,
