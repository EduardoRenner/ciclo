...
| `toque-48` em dois links dentro do MESMO parágrafo | A classe põe um `::after` absoluto de 48 px. Dois links inline que começam na mesma linha se cobrem: medido em `/cadastro`, o primeiro ficou com 49 px efetivos e o segundo com **zero** — intocável. Só use `toque-48` onde os elementos não dividem linha de texto corrida (uma `flex` com `gap`, como os rodapés da landing e de `/precos`). A guarda `alvo-de-toque-tem-48` não pega isto: ela confere se a classe está lá, não se o alvo resultante é alcançável. Sondagem ponto a ponto com `elementFromPoint`, receita no docstring dela. |
