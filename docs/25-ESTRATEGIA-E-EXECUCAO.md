...
| 5 | Freio antes do acelerador | Teto de envio por tenant/dia e interruptor por tenant, **antes** do passo 4. Um bug de laço com o motor ligado não custa erro na tela; custa a reputação do número de WhatsApp. | ✅ **Completo** — teto diário (`mensageria.ts`, `dentroDoTetoDiario`, via `limitador()`) e interruptor manual por tenant (`tenants.settings.messaging.paused`, toggle em `/admin/config/mensagens`, checado em `lembretes.ts`/`campaigns/route.ts`). |
| 6 | Alarme de silêncio | `cron_heartbeats` (migration 0014) já existe **[M]**. Falta alguém ser avisado quando um job para. Job que falha em silêncio é a armadilha nº 1 deste repositório (`21`). | ✅ Completo — `campaigns` grava heartbeat, `/api/health` checa os dois jobs com limiares próprios, e desde 26/08 (tarde) **só cobra execução de rota que está no `schedule`**: o endpoint estava em 503 permanente por causa de `send_reminders`, que ninguém dispara (`src/core/cron/agendadas.ts`, `DECISOES.md`). Com isso o step de CI deixou de depender do passo 4 — só precisa esperar o primeiro ciclo real (janela de 27/08) para não nascer vermelho. |
