...
| 18 | Recuperar vazio numa conta com fichas e sem atendimento dizia "O Motor começa no primeiro atendimento concluído" e levava para a agenda (semanas de espera); agora "Diga quando foi o último atendimento de cada um" → "Já atendo", e o Motor começa no mesmo dia | unidade (guarda antiga reescrita de propósito: afirmava a frase velha); medido a 375 px: ficha avulsa → link → busca → 1 toque |
| 18 | "Todo mundo em dia" passa a dizer "o próximo deve voltar daqui a N dias (dd/mm)" — mesma conta da tela do "Já atendo" (`quandoVolta`); `predicted_on >= hoje` porque `on_track` também cobre quem está atrasado mas remarcou | medido: as duas telas disseram "14 dias (07/10)" para a mesma ficha |
| 18 | Marcar só quem já tinha ficha terminava em "0 pessoas cadastradas" → "1 ficha atualizada" | medido a 375 px |
| 19 | "Hoje" de conta nova que pôs gente no Motor (ninguém atrasado) eram três pendências de custo e nenhuma palavra do Motor. A Central de Ações ganha, logo após o alarme de "sumindo": "Ninguém sumiu por enquanto — o próximo deve voltar daqui a N dias", ou, com fichas sem última visita, "Diga quando cada pessoa veio pela última vez" → "Já atendo" (`core/ciclo/acao-do-motor.ts`) | integração cobre os três estados; 3 mutações — a do corte pela data do salão passou cega na 1ª versão (faltava a ficha `on_track` com volta no passado, de quem remarcou); janela ampliada e caso posto no teste; medido a 375 px, 1º cartão da lista |
| 20 | §14 semana 2, "importar do caderno mais rápido": o "Já atendo" ganha **Colar uma lista** (um nome por linha, telefone na mesma linha entra junto, numeração/marcador saem, repetido não duplica, uma "última vez" para o lote) e **Escolher dos contatos** (Contact Picker API — só Chrome no Android; no iPhone o botão não aparece). Leitor puro em `core/ciclo/lista-de-nomes.ts`, com os tetos da rota | unidade 9/9, 5 mutações reprovadas; medido a 375 px: 4 linhas coladas (1 repetida) → 3 linhas com "Um mês" → "3 pessoas já estão atrasadas"; telefones gravados em E.164. **Contatos não testado em aparelho** (o navegador daqui não tem a API) |
| 21 | Home a 375 px: "Como começa" terminava em "Atenda. O resto o CICLO acompanha" (o caminho de semanas) — a porta de entrada era a única tela que ensinava a esperar. Passo 2 vira "Traga quem você já atende… aparece na lista no mesmo dia". A calculadora, que responde em dinheiro à pergunta do `h1`, sobe do rodapé para logo abaixo do botão | guarda nova `home-ensina-o-primeiro-dia` (1ª versão passou VAZIA: o extrator devolvia `''`; ganhou controle positivo) — 3 mutações reprovadas; link medido: 48 px, largura toda, `elementFromPoint` no centro e na borda |
| 21 | Conferido: a campanha diária automática (`/api/cron/campaigns`) está **fora** do agendador de propósito, então as fichas de memória que passam a atrasar (rodada 17) não disparam mensagem sozinhas. Quando ela for ligada, entram na rodada como qualquer outra | leitura do `cron.yml` |
| 22 | FAQ da home: "Dá, de uma planilha" e "Se você importar a data" — a planilha é a minoria do público. Agora: cole a lista ou escreva, "vale de memória", contatos no Android, planilha como alternativa; "fica útil no mesmo dia" | guarda estendida (`home-ensina-o-primeiro-dia`, com controle positivo do extrator da FAQ), mutação reprovada; texto conferido na página renderizada |
...
| 26 | Retomado a pedido ("continua /loop"), só com alvo seguro, sem mensagem automática nova. O bloqueio da exclusão de conta dizia "fale com o suporte para transferir" sem canal nenhum em produção: a frase vai para `core/auth/bloqueio-de-titular.ts`, só convida quando há canal, e a tela mostra o botão do canal junto. A guarda de travessão passa a varrer `src/server/services` e achou 4 frases de tela (1º passo do Hoje, recado da comanda, validação do custo fixo, o bloqueio) | 3 mutações reprovadas; a 1ª versão da varredura saiu VAZIA (`services/**/*.ts` exige subpasta no `git ls-files`) e o piso por nome pegou; botão do canal **não medido no navegador** (o ambiente local também não tem canal). **Commit só local: o PR #126 não o inclui até um push autorizado** |
| 27 | Revisão do código que entrou depois da última revisão (rodada 23), com foco na rodada 25 ("Chamar" vira o padrão): só a tela Recuperar usa `/cycle/recover/send`, então exigir `envio_em_lote` sempre não bloqueia nenhuma outra tela do grátis; "Chamar" registra a chamada (`anotarChamada`) com e sem telefone; a copy dos planos ("no grátis você manda um a um") continua verdadeira. **Nenhum defeito.** A guarda de travessão alcança também `src/lib` (cartões dos planos, mensagens prontas, contato) | mutação: travessão plantado em `planos-cartoes.ts` → reprova. Commit só local |
| 28 | Verificação no navegador do bloqueio de titular (rodada 26) esbarrou em MFA obrigatório em `/api/v1/account` (correto — é dado sensível). Fortaleci a prova no nível certo: `exclusao-de-conta.test.ts` agora afirma o texto exato do bloqueio contra Postgres local de verdade (sem canal configurado, igual a produção hoje), não só contra o teste de unidade da função pura | mutação (`bloqueioDeTitularSemSucessor(true)` fixo) → reprova; varredura de travessão em `public/*.txt`, `*.md` e providers de mensageria: nenhum achado novo |
| 35 | **Bio do Instagram e link da bio** (pedido do Eduardo). Página `/links` no domínio do produto, no lugar de uma linktree de terceiros: calculadora em destaque (o gancho "quanto você deixou de faturar"), depois criar conta, exemplo funcionando (some se não há demonstração no ar), como funciona e preços. Todo link leva `?origem=instagram`, e o link único da bio faz o middleware gravar o primeiro toque. Três variantes de bio (até 150 caracteres), nome do perfil (até 30) e ganchos de post e story em `docs/marketing/bio-e-link-da-bio.md` | unidade 14 casos; 6 mutações reprovadas (origem perdida, sem destaque, ordem trocada, bio acima de 150, "para sempre", exemplo sem demonstração), e a M2 só reprovou depois de eu perceber que a 1ª tentativa não tinha sido aplicada (CRLF); 4 guardas do projeto pegaram coisa real: **`links` faltava nos slugs reservados** (um salão com esse endereço perderia a página pública para sempre), o domínio estava escrito fora de `lib/app-url.ts`, o título repetia o nome do produto e a rota não estava na lista das institucionais; medido a 375 px (sem rolagem lateral, cartões de 78 px ou mais, rodapé com área de toque de 40 px ou mais por lado, claro mesmo com o aparelho em escuro, cookie `ciclo_origem=instagram` gravado). **Nada disso está no ar até o PR #126 ser mergeado** (`/links` e `/calculadora` dão 404 em produção). Slug `links`: conferido, `seuciclo.com.br/links` dava 404, então nenhum salão usa esse endereço (um salão com esse slug teria a página servida). **Revisão de copy e visual a 375 px (mesma rodada):** o título do cartão da calculadora quebrava em 3 linhas e o cadastro tinha o mesmo peso dos cartões secundários. Agora o destaque tem etiqueta "Comece por aqui" e o cadastro é o botão cheio, o único (contraste medido 5,03:1); a copy ficou como estava, sem promessa a mais. 2 mutações a mais reprovam (etiqueta some, dois botões principais) |
